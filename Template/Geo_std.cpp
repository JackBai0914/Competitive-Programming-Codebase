...
inline int sign(ld a) { return a < -EPS ? -1 : a > EPS; }
  
inline int cmp(ld a, ld b){ return sign(a-b); }
...
struct P {
	...
	ld dot(P p) { return x * p.x + y * p.y; }
	ld det(P p) { return x * p.y - y * p.x; }
	 
	ld distTo(P p) { return (*this-p).abs(); }
	ld alpha() { return atan2(y, x); }
	void read() { cin>>x>>y; }
	...
	P rot(ld an){ return {x*cos(an)-y*sin(an),x*sin(an) + y*cos(an)}; }
};
...
bool intersect(ld l1,ld r1,ld l2,ld r2){
...
ld area(vector<P> ps){
	ld ret = 0; rep(i,0,ps.size()) ret += ps[i].det(ps[(i+1)%ps.size()]); 
	return ret/2;
}
...
ld min_dist(vector<P>&ps,int l,int r){
	if(r-l<=5){
		ld ret = 1e100;
	...
	int m = (l+r)>>1;
	ld ret = min(min_dist(ps,l,m),min_dist(ps,m,r));
...
int type(P o1,ld r1,P o2,ld r2){
	ld d = o1.distTo(o2);
...
vector<P> isCL(P o,ld r,P p1,P p2){
	if (cmp(abs((o-p1).det(p2-p1)/p1.distTo(p2)),r)>0) return {};
	ld x = (p1-o).dot(p2-p1), y = (p2-p1).abs2(), d = x * x - y * ((p1-o).abs2() - r*r);
	d = max(d,0.0); P m = p1 - (p2-p1)*(x/y), dr = (p2-p1)*(sqrt(d)/y);
	return {m-dr,m+dr}; //along dir: p1->p2
}
  
vector<P> isCC(P o1, ld r1, P o2, ld r2) { //need to check whether two circles are the same
	ld d = o1.distTo(o2); 
	if (cmp(d, r1 + r2) == 1) return {};
	if (cmp(d,abs(r1-r2))==-1) return {};
	d = min(d, r1 + r2);
	ld y = (r1 * r1 + d * d - r2 * r2) / (2 * d), x = sqrt(r1 * r1 - y * y);
	P dr = (o2 - o1).unit();
	P q1 = o1 + dr * y, q2 = dr.rot90() * x;
	return {q1-q2,q1+q2};//along circle 1
}
  
vector<P> tanCP(P o, ld r, P p) {
	ld x = (p - o).abs2(), d = x - r * r;
...
vector<L> extanCC(P o1, ld r1, P o2, ld r2) {
	vector<L> ret;
	if (cmp(r1, r2) == 0) {
...
vector<L> intanCC(P o1, ld r1, P o2, ld r2) {
	vector<L> ret;
...
P circumCenter(P a, P b, P c) { 
	P bb = b - a, cc = c - a;
	double ld = bb.abs2(), dc = cc.abs2(), d = 2 * bb.det(cc);
	return a - P(bb.y * dc - cc.y * ld, cc.x * ld - bb.x * dc) / d;
}
